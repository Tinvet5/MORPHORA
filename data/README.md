...
## V3.2 navigation hierarchy

`catalog.json` registers species. Each available species points to a species data file under `data/species/`. Species data groups collections by anatomical system. Available collections then point to atlas manifests such as `atlas.json`.

Hierarchy:

Catalog → Species → System → Collection manifest → View JSON → Labels

## V4.2 discovery metadata

Optional species fields:
...
These fields control discovery cards, global search, and coming-soon pages. They do not affect the atlas viewer itself.
