document.addEventListener("DOMContentLoaded", () => {
  ...
  function showImageSourceLoadingState(viewData, { fallback = false } = {}) {
    ...
    showViewerState({
      ...
      title: `Preparing ${viewData?.title || "anatomical view"}`,
      message: fallback
    ...
    const views = data.views.map((entry, index) => {
      ...
      if (entry.thumbnail !== undefined) {
        requireData(
          ...
          `Atlas view “${entry.id}” requires thumbnail to be a non-empty path when provided.`
        );
      }
  ...
  function validateViewData(data, manifestEntry) {
    ...
    requireData(
      imageType === "image" || imageType === "dzi",
      `View “${data.id}” image.type must be “image” or “dzi”.`
    );

    if (data.image.fallback !== undefined) {
      requireData(
        typeof data.image.fallback === "string" && data.image.fallback.trim(),
        `View “${data.id}” requires image.fallback to be a non-empty path when provided.`
      );
    }

    if (data.image.thumbnail !== undefined) {
      requireData(
        typeof data.image.thumbnail === "string" && data.image.thumbnail.trim(),
        `View “${data.id}” requires image.thumbnail to be a non-empty path when provided.`
      );
    }

    ["width", "height"].forEach((dimension) => {
      if (data.image[dimension] === undefined) return;
      requireData(
        Number.isFinite(Number(data.image[dimension])) && Number(data.image[dimension]) > 0,
        `View “${data.id}” requires image.${dimension} to be a positive number.`
      );
    });
    ...
    const labels = data.labels.map((label, index) => {
      ...
      const quizEligible = label.quizEligible !== false;
      const difficulty = ["beginner", "intermediate", "advanced"].includes(label.difficulty)
        ? label.difficulty
        : "intermediate";
      const acceptedRadiusValue = Number(label.acceptedRadius);
      const acceptedRadius = Number.isFinite(acceptedRadiusValue)
        ? acceptedRadiusValue
        : null;

      if (acceptedRadius !== null) {
        requireData(
          acceptedRadius >= 0.005 && acceptedRadius <= 0.2,
          `Label “${label.id}” in “${data.id}” requires acceptedRadius between 0.005 and 0.2.`
        );
      }
      ...
      return {
        ...
        quizEligible,
        difficulty,
        ...(acceptedRadius !== null ? { acceptedRadius } : {})
      };
    });
    ...
    return {
      ...
      image: {
        ...
        ...(typeof data.image.fallback === "string"
          ? { fallback: data.image.fallback.trim() }
          : {}),
        ...(typeof data.image.thumbnail === "string"
          ? { thumbnail: data.image.thumbnail.trim() }
          : {})
      },
      labels
    };
  ...
  async function prefetchViewEntry(entry) {
    ...
    let viewData = viewDataCache.get(cacheKey);
  ...
  viewer.addHandler("open", (event) => {
    ...
    if (activeImageLoad?.useFallback) {
      A11y.announce("High-resolution tiles were unavailable. MORPHORA opened the compatible fallback image.");
    }
  ...
  viewer.addHandler("tile-loaded", () => {
    if (!activeViewData || !activeImageLoad || activeImageLoad.firstTileMarked) return;
